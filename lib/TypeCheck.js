...
module.exports = function() {
    ...
    var validatorConfigSyntaxError = function(error) {
        return {
            'status': 'error',
            'error': {
                'code': cls.EXCEPTION.COMPILE_ERROR_VALIDATOR_CONFIG_SYNTAX,
                'details': error
            }
        };
    };

    var scopeSyntaxError = function(error) {
        return {
            'status': 'error',
            'error': {
                'code': cls.EXCEPTION.COMPILE_ERROR_SCOPE_SYNTAX,
                'details': error
            }
        };
    };

    var scopeAliasError = function(error) {
        return {
            'status': 'error',
            'error': {
                'code': cls.EXCEPTION.COMPILE_ERROR_SCOPE_ALIAS,
                'details': error
            }
        };
    };
    ...
    cls.prototype = {
        ...
        compile: function() {
            ...
            if (res['status'] == 'error') {
                ...
                return validatorConfigSyntaxError(res['error']);
            }
            ...
            if (res['status'] == 'error') {
                return scopeSyntaxError(res['error']);
            }

            res = this._compile.compile();

            if (res['status'] == 'error') {
                return scopeAliasError(res['error']);
            }
    ...
    $U.extend(cls, {
        EXCEPTION: {
            ...
            /* compile */
            COMPILE_ERROR_VALIDATOR_CONFIG_SYNTAX: 'compile_error_validator_config_syntax',
            COMPILE_ERROR_SCOPE_SYNTAX: 'compile_error_scope_syntax',
            COMPILE_ERROR_SCOPE_ALIAS: 'compile_error_scope_alias'
        },
        merge: merge
    });
