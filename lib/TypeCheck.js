    ...
    cls.protoypte = {
        ...
        _typeCheck: function(value, type, scopeStack) {
            ...
            var validate = function(type, value) {
                if (type['validator']) {
                    for (var validatorName in type['validator']) {
                        var o = me._validators[validatorName];
                        var cfg = type['validator'][validatorName];
                        if (!o.check(value, type['cfg'])) {
            ...
            switch(typeTag) {
                ...
                case 'Alias':
                    var res = scopeStack.lookupType(type['alias'], type['path'] || []);
                    if (res) {
