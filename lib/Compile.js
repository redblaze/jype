...
cls.prototype = {
    ...
    checkNoCycle: function(scope) {
        for (var k in scope) {
            var type = scope[k];
            var res = this._checkNoCycle([], k, type, scope);
            if (res['status'] == 'error') {
                return res;
            }
        }
    ...
    _checkNoCycle: function(path, name, type, scope) {
        if ($U.contains(path, name)) {
            return cycleError($U.clone(path).concat([name]));
        }

        if (type['type'] == 'Alias') {
            path.push(name);
            var newName = type['name'];
            var newType = scope[newName];
            var res = this._checkNoCycle(path, newName, newType, scope);
            path.pop();
            return res;
        } else if (type['type'] == 'Union') {
            for (var k in type['variants']) {
                var newType = type['variants'][k];
                var res = this._checkNoCycle(path, name, newType, scope);
                if (res['status'] == 'error') {
                    return res;
                }
            }
